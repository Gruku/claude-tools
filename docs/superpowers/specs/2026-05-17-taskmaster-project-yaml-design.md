...
5. **CodeMaestro is the first real population.** Author its `project.yaml` as the canonical example; commit alongside the harvest's downstream work. Canonical example shipped in CodeMaestro at `.taskmaster/project.yaml` (commit `9d0d594` on master).
