---
name: yaml
description: Generate UE5 Material Graph nodes from YAML definitions. Direct workflow that skips the agent chain. For simple node setups where you know exactly what nodes you need.
---

# UE5 Shader Node Generator (Direct YAML)

Generate material graph nodes ready to paste into UE5 Material Editor. This is the direct path -- write YAML, run the converter, paste. No agents involved.

For complex shaders where you need HLSL design help, use `/shader` instead.

## Workflow

1. **You provide**: Shader code or description of what you need
2. **Claude generates**: YAML definition
...
To add new node types, use `/shader-registry [NodeTypeName]` or edit `../../node_registry.yaml` directly.
