---
name: ue5-node-registry
description: Check if a UE5 material node type exists in the registry. If not, create and add it. Use this BEFORE generating YAML when you encounter an unfamiliar node type.
user-invocable: true
argument-hint: [node type name, e.g., "Fresnel", "SphereMask"]
---
...
```
~/.claude/skills/ue5-custom-shader-nodes/node_registry.yaml
```
...
```python
# Check ~/.claude/skills/ue5-custom-shader-nodes/node_registry.yaml
```
...
```yaml
DepthFade:
  expression_class: MaterialExpressionDepthFade
  inputs:
    - name: Opacity
      category: optional
    - name: FadeDistance
...
```yaml
Panner:
  expression_class: MaterialExpressionPanner
  inputs:
    - name: Coordinate
      category: optional
    - name: Time
...
```yaml
Rotator:
  expression_class: MaterialExpressionRotator
  inputs:
    - name: Coordinate
      category: optional
    - name: Time
